        ...
        >
          ...
          >
            <template #default="{ row }">
              <span class="server-address">{{ row.ip }}</span>
            </template>
          </el-table-column>
...
import type { Aria2Connection } from '@/types'
import {aria2ServerApi} from "@/api/views/aria2_server";
...
export default defineComponent({
  ...
  setup() {
    const connectionList = ref<Aria2Connection[]>([])
    const loading = ref(false)
    const dialogVisible = ref(false)
    const isEdit = ref(false)
    const formRef = ref<FormInstance | null>(null)

    const connectionForm = ref<Partial<Aria2Connection>>({
      ip: '',
      port: 6800,
      secret: '',
      weight: 1
    ...
    const loadConnections = async () => {
      loading.value = true
      try {
        const response = await aria2ServerApi.getConnections()
        if (response.data.code === 200) {
          connectionList.value = response.data.data
        } else {
    ...
    const handleEdit = (row: Aria2Connection) => {
    ...
    const handleSubmit = async () => {
      ...
      await formRef.value.validate(async (valid) => {
        ...
        try {
          let response
          if (isEdit.value) {
            response = await aria2ServerApi.updateConnection(connectionForm.value.id!, {
              ip: connectionForm.value.ip!,
              port: connectionForm.value.port!,
              secret: connectionForm.value.secret!,
              weight: connectionForm.value.weight!
            })
          } else {
            response = await aria2ServerApi.addConnection({
              ip: connectionForm.value.ip!,
              port: connectionForm.value.port!,
              secret: connectionForm.value.secret!,
              weight: connectionForm.value.weight!
    ...
    const handleDelete = async (row: Aria2Connection) => {
      try {
        const response = await aria2ServerApi.deleteConnection(row.id)
        if (response.data.code === 200) {
